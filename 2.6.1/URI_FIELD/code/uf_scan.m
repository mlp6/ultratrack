...
%
rfdata=zeros(1,beamset.no_beams,beamset.no_beamsy);
...
%	uf_set_beam(tx,rx,probe,beamset,1,n_vector);
	uf_set_beam(tx,rx,probe,beamset,1,n_vector,m_vector);
        ...
        [red_phantom]=reduce_scats(phantom,beamset.originx(n_vector),beamset.originy(m_vector),search_radius);

	[v,t1]=calc_scat(tx,rx,red_phantom.position,red_phantom.amplitude);
	if (size(rfdata,1)<length(v)),
		disp('Memory');
		rfdata=[rfdata ;zeros(length(v)-size(rfdata,1),beamset.no_beams,beamset.no_beamsy) ];
	end;
	rfdata(1:length(v),n_vector,m_vector)=v;
	start_times(n_vector,m_vector)=t1;
   end;
end;
whos rfdata
