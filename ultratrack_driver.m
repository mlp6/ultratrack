function []=ultratrack_driver(varargin)
% function []=bme790_ultratrack_driver(phantom_seed)
% INPUTS:
%   phantom_seed (int) - scatterer position RNG seed
%   DEST_DIR (string) - optional path to save location (default is pwd)
% OUTPUTS:
%   Nothing returned, but lots of files and directories created in PATH
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% this is branched off of v2.6.1 to work for Kathy's BME790 S13 class
% URI toolbox calls removed
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%  --------------PARSE INPUTS---------------------------------------
if nargin > 2
    phantom_seed = varargin{1};
    delta = varargin{2};
    DEST_DIR = varargin{3};
elseif nargin == 2
    phantom_seed = varargin{1};
...
%%  --------------IMAGING PARAMETERS---------------------------------
PARAMS.PROBE ='vf7-3';
PARAMS.COMPUTATIONMETHOD = 'parfor'; % 'cluster','parfor', or 'none'

probe=uf_txt_to_probe([PARAMS.PROBE '.txt']);
...
PARAMS.TX_FOCUS= 20e-3;    % Transmit focus depth (m)
PARAMS.TX_F_NUM=[1.5 1.5];      % Transmit f number (the "y" number only used for 2D matrix arrays)
PARAMS.TX_FREQ=6.15e6;      % Transmit frequency (Hz)
PARAMS.TX_NUM_CYCLES=2;     % Number of cycles in transmit toneburst
PARAMS.TX_APOD_TYPE=1;      %Transmit apodization (0 = none, 1 = hamming)
PARAMS.RX_FOCUS= 0;          % Depth of receive focus - use 0 for dyn. foc
PARAMS.RX_F_NUM=[0.75 0.75];  % Receive aperture f number (the "y" number only used for 2D matrix arrays)
PARAMS.RX_GROW_APERTURE=1;  %Receive aperture growth (if true, constant F/# through depth)
PARAMS.RX_APOD_TYPE=1;      %Receive apodization (0 = none, 1 = hamming)
PARAMS.MINDB = -40; %minimum contribution for including a scatter in sensitivity-based reduction
PARAMS.GRIDSPACING = [0.5e-3 0.5e-3 0.5e-3]; %Spacing of grid for sensitivity-based scatterer reduction (m)
...
TRACKPARAMS.MAX_SEARCH = 30e-6; %Size of search window in meters (defines the largest trackable displacement)
TRACKPARAMS.PRE_INTERP_FACTOR = 1; %RF interpolation factor applied prior to displacement estimator
TRACKPARAMS.CORR_SPARSENESS = 5; %Axial (fast-time) downsample factor (use this to reduce the data set and speed up total processing time)


%% ------------------PHANTOM PARAMETERS----------------------------
% file containing comma-delimited node data
DYN_FILE='nodes.dyn'; % <- CHANGE THIS TO YOUR SIMS'S nodes.dyn FILE
DEST_DIR = fullfile(DEST_DIR);
ZDISPFILE = fullfile(DEST_DIR,'disp.dat');

% Read the .dyn file to get node positions
[~,X,Y,Z]=read_dot_dyn(DYN_FILE);
...
PPARAMS.ymax=max(Y(:));  
PPARAMS.zmin= min(Z(:));
PPARAMS.zmax=-0.1;%max(Z(:));  

%Account for symmetry condition with the min and max coordinates (these are
%used later to determine the bounds of the ultrasound phantom)
if strcmp(PPARAMS.SYM,'q')
    PPARAMS.xmax = -(PPARAMS.xmin);
    PPARAMS.ymin = -(PPARAMS.ymax);
end
...
NUM_SCAT_PER_RES_CELL = 11;
lambda = PARAMS.c/PARAMS.TX_FREQ; %m

%Calculate rough estimate of resolution cell size (lambda*F#)
RES_CELL_LAT = lambda*mean([PARAMS.RX_F_NUM(1) PARAMS.TX_F_NUM(1)])*1e2; %cm. Take the mean F/# used between TX and RX 
RES_CELL_ELE = lambda*mean([PARAMS.RX_F_NUM(2) PARAMS.TX_F_NUM(2)])*1e2; %cm.
RES_CELL_AX  = lambda*1e2; %cm

SCATTERER_DENSITY = NUM_SCAT_PER_RES_CELL/(RES_CELL_LAT*RES_CELL_ELE*RES_CELL_AX); % scatterers/cm^3  
TRACKING_VOLUME = (PPARAMS.xmax-PPARAMS.xmin)*(PPARAMS.ymax-PPARAMS.ymin)*(PPARAMS.zmax-PPARAMS.zmin); % cm^3

%Compute number of scatterers
...
if USE_POINT_SCATTERERS
    ...
    PPARAMS.pointscatterers.a = 1.0; % Point scatterer amplitude
end

PPARAMS.delta=delta;   % rigid pre-zdisp-displacement scatterer translation,
...
PHANTOM_DIR=fullfile(DEST_DIR,make_file_name('phantom',P));
regeneratephantom = 0;
d = dir(fullfile(PHANTOM_DIR,'phantom*.mat'));
RF_DIR=fullfile(DEST_DIR,make_file_name('rf',P));
RF_FILE=fullfile(RF_DIR,'rf');
if isempty(d) || regeneratephantom
    mkdir(PHANTOM_DIR); %matlab 7
    mkphantomfromdyna3symmetry(DYN_FILE,ZDISPFILE,fullfile(PHANTOM_DIR,'phantom'),PPARAMS);
    %mkphantomwithflow(DYN_FILE,ZDISPFILE,fullfile(PHANTOM_DIR,'phantom'),PPARAMS);
end
...
do_dyna_scans(PHANTOM_DIR,RF_FILE,PARAMS);
...
[arfidata,correlation,axial]=estimate_disp(double(bigRF),PARAMS,TRACKPARAMS);

%% save res_tracksim.mat (same format as experimental res*.mat files)
TRACKEDFILENAME = fullfile(RF_DIR,['res_tracksim_' num2str(TRACKPARAMS.WAVELENGTHS) 'lambda.mat']);
save(TRACKEDFILENAME,'arfidata','correlation','axial','PARAMS','TRACKPARAMS');
