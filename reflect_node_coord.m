function [Xr, Yr, Zr] = reflect_node_coord(sym, X, Y, Z, debug);
% function [Xr, Yr, Zr] = reflect_node_coord(sym, X, Y, Z, debug);
%
...
%
% INPUTS:   sym (string) - 'q' (quarter), 'h' (half) symmetry
...
switch sym
    case 'h'
        ...
        Xr = cat(1, X, -flipdim(X(1:end-1,:,:), 1));
    ...
    case 'q'
        ...
        Yr = cat(2, -flipdim(Yr(:,2:end,:), 2), Yr);
