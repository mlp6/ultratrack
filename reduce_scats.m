function [phantom]=reduce_scats(phantom,scan_lat_pos,scan_el_pos,search_radius)
...
%           scan_el_pos (single) - scan elevational position (m)
...
disp(sprintf('Elevational Position: %.2f mm',scan_el_pos*1e3));
phantom_radii = sqrt((phantom.position(:,1)-scan_lat_pos).^2 + (phantom.position(:,2)-scan_el_pos).^2);
...
disp(sprintf('Elevational Range: %.3f - %.3f mm',min(phantom.position(:,2))*1e3,max(phantom.position(:,2))*1e3));
disp('-------------------------------------------');
