function []=pas(Smark,Freq,KernelSize,TxOffset,PATH)
% function []=pas(Smark,Freq,KernelSize,TxOffset,PATH)
% INPUTS:	Smark - scatterer distribution seed
% 		Freq - frequency (MHz)
%			KernelSize - # samples in tracking kernel (default = 35)
%     TxOffset (float) - offset of Tx beam from Rx beam (m)
...
% Mark 06/17/05
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Added TxOffset as an input parameter.
% Mark 06/20/05
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% ADD PATH TO MARK'S TRACKING CODE
addpath('/home/mlp6/matlab/UltrasonicTracking');
...
% single line data
PPARAMS.ymin=[-0.2];PPARAMS.ymax=[0.2];	% lateral, cm \
% full FOV data
...
switch Freq
case 7,
  %PARAMS.PROBE_NAME='vf7-3.txt'; % Probe text file name
  PARAMS.PROBE_NAME='vf10-5.txt'; % Probe text file name
otherwise,
  PARAMS.PROBE_NAME='vf7-3.txt'; % Probe text file name
end;
PARAMS.XMIN=-0.001;		% Leftmost scan line (m)
PARAMS.XSTEP=0.0002;	% Scanline spacing (m)
PARAMS.XMAX=0.001;		% Rightmost scan line (m)
%PARAMS.XMIN=-0.1e-1;		% Leftmost scan line (m)
%PARAMS.XSTEP=2e-4;		% Scanline spacing (m)
%PARAMS.XMAX=0.1e-1;		% Rightmost scan line (m)
