...
DEST_DIR = PATH;
%DEST_DIR='/moredata/mlp6/CIRS_FEM/LesionSims/t14mm_d3mm_E15kPa_foc15mm_a0p72/F4p0/les14mm1p2mmE15kPa_foc15mm_a0p72/'
ZDISPFILE = [DEST_DIR 'zdisp.mat'];
%ZDISPFILE='/emfd/mlp6/CIRS_FEM/LesionSims/t14mm_d3mm_E15kPa_foc15mm_a0p72/les14mmE15kPa_foc15mm_a0p72/zdisp.mat'
PPARAMS.N=10000;
%PPARAMS.N=2500;

PPARAMS.delta=DELTA;  % rigid pre-zdisp-displacement scatterer translation,
%PPARAMS.delta=[0 0.12 0]  % rigid pre-zdisp-displacement scatterer translation,
                          % in the dyna coordinate/unit system

				% leave any empty to use mesh limit
PPARAMS.xmin=[-0.15];PPARAMS.xmax=[0];		% out-of-plane,cm
PPARAMS.ymin=[-0.4];PPARAMS.ymax=[0.4];	% lateral, cm \
PPARAMS.zmin=[];PPARAMS.zmax=[-0.1];		% axial, cm   / X,Y SWAPPED vs FIELD!	
PPARAMS.TIMESTEP=[1:31];		% Timesteps to simulate.  Leave empty to
...
PARAMS.PROBE_NAME='vf10-5.txt'; % Probe text file name
PARAMS.XMIN=-3.0e-3;		% Leftmost scan line (m)
PARAMS.XSTEP=2e-4;		% Scanline spacing (m)
PARAMS.XMAX=3.0e-3;		% Rightmost scan line (m)
