...
if(geometry.image_mode == 'linear'),
    xdc_center_focus(Tx,[beamset(set).origin(vector,1)+txoffset(1) txoffset(2) 0] );
...
tx_ap_left_limit =-tx_width/2+beamset(set).origin(vector,1)+txoffset(1);
tx_ap_right_limit= tx_width/2+beamset(set).origin(vector,1)+txoffset(1);
