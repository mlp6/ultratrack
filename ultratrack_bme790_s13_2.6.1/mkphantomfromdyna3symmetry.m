...
%    sdX = nan(size(scatterers(:,1)));sdY = sdX;sdZ = sdX;
    
%   idxX = (scatterers(:,1) >= min(X(:))) & (scatterers(:,1) <= max(X(:)));
   idxnX = (scatterers(:,1) >= min(-X(:))) & (scatterers(:,1) < max(-X(:)));
%   idxY = (scatterers(:,2) >= min(Y(:))) & (scatterers(:,2) <= max(Y(:)));
   idxnY = (scatterers(:,2) >= min(-Y(:))) & (scatterers(:,2) < max(-Y(:)));
%   idxZ = (scatterers(:,3) >= min(Z(:))) & (scatterers(:,3) <= max(Z(:)));
   idxnZ = (scatterers(:,3) >= min(-Z(:))) & (scatterers(:,3) < max(-Z(:)));
  
        sdX= (-1).^idxnX .* interpn(X,Y,Z,dX,...
            scatterers(:,1).*(-1).^idxnX,...
            scatterers(:,2).*(-1).^idxnY,...
            scatterers(:,3).*(-1).^idxnZ,'linear');
        sdY= (-1).^idxnY .* interpn(X,Y,Z,dY,...
            scatterers(:,1).*(-1).^idxnX,...
            scatterers(:,2).*(-1).^idxnY,...
            scatterers(:,3).*(-1).^idxnZ,'linear');
        sdZ= (-1).^idxnZ .* interpn(X,Y,Z,dZ,...
            scatterers(:,1).*(-1).^idxnX,...
            scatterers(:,2).*(-1).^idxnY,...
            scatterers(:,3).*(-1).^idxnZ,'linear');
